...
import { listRehomePlants, Plant } from "@/lib/plants";

export const Route = createFileRoute("/utflyttade")({
  head: () => ({
    meta: [{ title: "Utflyttade – Min Växtdagbok" }],
  }),
  component: UtflyttadePage,
});
      ...
      <header className="sticky top-0 z-10 border-b border-border bg-background/85 backdrop-blur">
        <div className="mx-auto max-w-3xl px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold tracking-tight">Utflyttade</h1>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-3xl px-4 pt-6">
        {plants === null ? (
          ...
          <div className="mt-16 text-center">
            ...
            <h2 className="mt-4 text-lg font-medium">Inga utflyttade växter</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Växter du gett bort hamnar här.
            </p>
          </div>
        ) : (
          <>
            <p className="mb-5 text-sm text-muted-foreground">
              {plants.length === 1 ? "1 växt" : `${plants.length} växter`} har hittat nya hem.
            </p>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              {plants.map((p) => (
                <Link
                  ...
                  <div className="px-3 py-2.5">
                    ...
                    {p.status_changed_at && (
                      <div className="truncate text-xs text-muted-foreground">
                        Utflyttad · {formatDate(p.status_changed_at)}
                      </div>
                    )}
                    {p.status_note && (
